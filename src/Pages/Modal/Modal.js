            ...
            <div className={active ? "modal active" : "modal"} onClick={() => setActive(false)}>
                <div className="modal--content" onClick={e => e.stopPropagation()}>
                    <div className="modal--content__h3">
                        ...
                        <h3 className={active ? "modal active " : "modal"} onClick={() => setActive(false)}><MdClose/></h3>
                    </div>
                    <h2>Биз сиз менен байланышабыз.</h2>
                    <input value={value.fullName} name='fullName' onChange={handleInputValue} type="text" placeholder="Аты-жөнү" required/>
                    <input value={value.email} name="email" onChange={handleInputValue} type="email" placeholder="Gmail "required/>
                    <textarea value={value.message} name="message" onChange={handleInputValue} className="text" placeholder="Комментарий"/>
